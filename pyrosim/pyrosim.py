...
def Send_Link(name,pos,size,objectType,mass,materialName,colorRgba):
    ...
    if filetype == SDF_FILETYPE:
        ...
        link = LINK_URDF(name,pos,size,materialName,colorRgba)
...
def Send_Cube(name="default",pos=[0,0,0],size=[1,1,1],mass=1.0,materialName="Cyan",colorRgba="0 1.0 1.0 1.0"):

    Send_Link(name,pos,size,"box",mass,materialName,colorRgba)
